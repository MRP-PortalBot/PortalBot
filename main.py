...
client.slash = SlashCommand(client,
                            sync_commands=True)  #TODO: Fix Slash Commands
...
async def slashm(ctx):
	embed = discord.Embed(title="Slash Management Commands",
	                      description="Slash Commands",
	                      color=0x1ebdf7)
	embed.add_field(
	    name="Commands:",
	    value=
	    "**get** - Get's current slash commands in the API \n**remove** - Remove's slash commands from the API. (Needs commandid and guildid)"
	)
	await ctx.send(embed=embed)
...
async def get(ctx, guildid=None):
	if guildid == None:
		...
		await ctx.send("```\n" + str(await (manage_commands.get_all_commands(
		    client.user.id, os.getenv("TOKEN"), guildid))) + "\n```")
	else:
		await ctx.send("```\n" + str(await (manage_commands.get_all_commands(
		    client.user.id, os.getenv("TOKEN"), guildid))) + "\n```")
...
async def remove(ctx, commandid, guildid=None):
	try:
		await ctx.send(
		    "Response: " + str(await (manage_commands.remove_slash_command(
		        config['BotID'], os.getenv("TOKEN"), guildid, commandid))))
	except:
...
async def shell(ctx, *, command=None):
	...
	try:
		...
		embed = discord.Embed(
		    title="Shell Process",
		    description=f"Shell Process started by {author.mention}",
		    color=0x4c594b)
