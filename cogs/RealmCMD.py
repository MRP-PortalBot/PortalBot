...
class RealmCMD(commands.Cog):
  ...
  async def newrealm(self, ctx, realm, emoji,  user: discord.Member, *, message=None):
    ...
    await channelrr.send(role.mention + " Please agree to the rules to gain access to the Realm Owner Chats!")
  ...
  async def newrealm_error(self, ctx,error):
    if isinstance(error, commands.MissingPermissions):
      ctx.send("Uh oh, looks like I can't execute this command because you don't have permissions!")

    if isinstance(error, commands.TooManyArguments):
      ctx.send("You sent too many arguments! Did you use quotes for realm names over 2 words?")
