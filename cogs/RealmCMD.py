...
sheet = client.open("Minecraft Realm Portal Channel Application (Responses)").sheet1
...
class RealmCMD(commands.Cog):
  ...
  async def checkin2(self,ctx):
    #28
    em = discord.Embed(title = "Realm Checkin", description = "Please react with your Realm Emoji to checkin for the month!\n======================================================")
    em.add_field(name = "Page 1/2", value = 
    "77th Combine - 🚜\n"
    ...
    #Part2
    em = discord.Embed(title = "Realm Checkin", description = "Please react with your Realm Emoji to checkin for the month!\n======================================================")
    em.add_field(name = "Page 2/2", value = 
    "Fortressworld - 🐉\n"
    "Fresh Start - 🍃\n"
    "Genesis - 🌱\n"
    "Hals Crafters - 🌞\n"
  ...
  async def applyrealm(self, ctx):
    ...
    if channel2.name != "bot-spam":
      ...
      noGoAway = discord.Embed(title = "Woah Woah Woah, Slow Down There Buddy!", description = "Please switch to #bot-spam! This command is not allowed to be used here.", color = 0xfc0b03)
    ...
    Level Check
    '''
    JustSpawnedCheck = discord.utils.get(ctx.guild.roles, name="Just Spawned")
    SpiderSniperCheck = discord.utils.get(ctx.guild.roles, name="Spider Sniper")
    if JustSpawnedCheck in author.roles or SpiderSniperCheck in author.roles:
      noGoAway = discord.Embed(title = "Woah Woah Woah, Slow Down There Buddy!", description = "I appreciate you trying to apply towards a new channel here but you must have the `Zombie Slayer` role or have surpassed this role! \n*Please try again when you have reached this role.*", color = 0xfc0b03)
    ...
    embed = discord.Embed(title = "Realm Application", description = "Response turned in by: " + author.mention + "\nRealm Name: " + answer1.content, color = 0x03fc28)
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/588034623993413662/588413853667426315/Portal_Design.png")
    embed.add_field(name="__**Realm Name**__", value=str(answer1.content), inline=True)
    embed.add_field(name="__**Emoji**__", value=str(answer2.content), inline=True)
    embed.add_field(name="__**Short Description**__", value=str(answer3.content), inline=False)
    embed.add_field(name="__**Long Description**__", value=str(answer4.content), inline=False)
    embed.add_field(name="__**Application Process**__", value=str(answer5.content), inline=False)
    embed.add_field(name="__**Current Member Count**__", value=str(answer6.content), inline=True)
    embed.add_field(name="__**Age of Realm/Server**__", value=str(answer7.content), inline=True)
    embed.add_field(name="__**Will your Realm have the ability to continue for the foreseeable future?**__", value=str(answer8.content), inline=False)
    embed.add_field(name="__**Members of the OP Team (Owner First)**__", value=str(answer9.content), inline=False)
    ...
    #Confirmation
    response = discord.Embed(title = "Success!", description = "I have sent in your application, you will hear back if you have passed!", color = 0x03fc28)
