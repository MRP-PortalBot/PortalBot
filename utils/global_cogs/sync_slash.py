    ...
    try:
        if isinstance(action, discord.Guild):
                ...
                try:
                    ...
                    await interaction.followup.send(
                        embed=discord.Embed(
                            color=discord.Color.green(),
                            title="Sync Complete",
                            description=f"✅ Successfully synced `{action}` slash commands.",
                        )
                    )
                    _log.info(f"Successfully completed {action} sync.")

                except Exception as e:
                    _log.error(f"Error during {action} sync: {e}")
                    await interaction.followup.send(
                        embed=discord.Embed(
            ...
            embed_confirm = discord.Embed(
                ...
                title="Sync Confirmation",
                description=(
                    "Are you sure you want to sync **globally**? This may take up to 1 hour."
                    if action == "global"
        ...
        _log.error(f"An error occurred during sync operation: {e}", exc_info=True)
        await ctx.send(
            embed=discord.Embed(
                title="Sync Error",
                description=f"An error occurred while syncing: `{e}`",
                color=discord.Color.red(),
            )
        )
