...
async def generate_realm_profile_card(interaction: discord.Interaction, realm_name: str):
    ...
    try:
        ...
        banner = Image.open(profile.banner_url).convert("RGBA") if os.path.exists(profile.banner_url) else Image.open(fallback_banner).convert("RGBA")
        logo = Image.open(profile.logo_url).convert("RGBA").resize((200, 200)) if os.path.exists(profile.logo_url) else fallback_logo
        ...
        await interaction.followup.send(file=discord.File(buffer, "realm_profile_card.png"))
        _log.info(f"Realm profile card generated for {realm_name}")
    except Exception as e:
        _log.error(f"Failed to generate card: {e}", exc_info=True)
        await interaction.followup.send("Error generating profile card.", ephemeral=True)


async def update_realm_logo_attachment(interaction: discord.Interaction, realm_name: str, attachment: discord.Attachment):
    try:
        if not attachment.content_type or not attachment.content_type.startswith("image"):
            await interaction.response.send_message("Only image files are supported.", ephemeral=True)
            return
        ...
        if profile:
            ...
            profile.save()
            await interaction.response.send_message("✅ Realm logo uploaded successfully.", ephemeral=True)
        else:
            await interaction.response.send_message("⚠️ Realm not found.", ephemeral=True)
    except Exception as e:
...
async def update_realm_banner_attachment(interaction: discord.Interaction, realm_name: str, attachment: discord.Attachment):
    try:
        if not attachment.content_type or not attachment.content_type.startswith("image"):
            await interaction.response.send_message("Only image files are supported.", ephemeral=True)
            return
        ...
        if profile:
            ...
            profile.save()
            await interaction.response.send_message("✅ Realm banner uploaded successfully.", ephemeral=True)
        else:
            await interaction.response.send_message("⚠️ Realm not found.", ephemeral=True)
    except Exception as e:
        _log.error(f"Failed to upload banner: {e}", exc_info=True)
        await interaction.response.send_message("Error uploading banner.", ephemeral=True)
