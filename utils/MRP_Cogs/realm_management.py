...
class RealmCMD(commands.Cog):
    ...
    async def create_realm(self, interaction: discord.Interaction, app_number: int):
        ...
        try:
            ...
            embed.add_field(name="**Console Logs**", value="**Role Created:** " + role_create + " -> " + role.mention + "\n**Channel Created:** " + channel_create +" -> <#" + str(channel.id) + ">\n**Role Given:** " + role_given + "\n**Channel Permissions:** " + channel_permissions + "\n**DMStatus:** " + DMStatus)
    ...
    async def apply_for_a_realm(
        ...
        view = return_applyfornewrealm_modal(self.bot, realm_name, type_of_realm, emoji, str(member_count), community_duration, world_duration, reset_schedule)
