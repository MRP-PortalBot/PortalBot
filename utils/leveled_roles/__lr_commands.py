...
from admin.admin_core.__admin_commands import has_admin_level
...
class LeveledRolesSetup(commands.GroupCog, name="leveledroles"):
    ...
    @app_commands.command(name="setup", description="Create and order leveled roles for this server.")
    @has_admin_level(3)
    async def setup_roles(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("✅ Leveled roles have been set up.")
        except discord.Forbidden:
            await interaction.response.send_message("I don't have permission to manage roles.", ephemeral=True)
        except discord.HTTPException as e:
            await interaction.response.send_message(f"Error setting up roles: {e}", ephemeral=True)
